<template>
  <div class="card">    
        ...
        <DataTable ref="dt" :value="profesores" v-model:selection="selectedProfesores" dataKey="id" 
            ...
            currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} Docentes" responsiveLayout="scroll">
            <template #header>
...
export default {
    ...
    methods: {
        ...
        async exportToPDF() {
            ...
            doc.text('Lista de Docentes', 83, 28);
            // Cabeceras y orden de las columnas
            const headers = [['Nombre', 'A. Paterno', 'A. Materno', 'DNI', 'Teléfono', 'Especialidad', 'Dirección', 'Estado']];
            // Prepara los datos para la exportación
            const dataToExport = this.profesores.map(row => [
                row.pro_nom || '',
                row.pro_app || '',
                row.pro_apm || '',
                row.pro_dni || '',
                row.pro_telf || '',
                row.pro_especialidad || '',
                row.pro_dire || '',
                row.pro_estado === 1 ? 'Activo' : 'Inactivo'
            ]);
        ...
        exportToExcel() {
            ...
            const dataToExport = this.profesores.map(row => ({            
                'Nombre': row.pro_nom || '',
                'A. Paterno': row.pro_app || '',
                'A. Materno': row.pro_apm || '',
                'DNI': row.pro_dni || '',
                'Teléfono': row.pro_telf || '',
                'Especialidad': row.pro_especialidad || '',
                'Dirección': row.pro_dire || '',
                'Estado': row.pro_estado === 1 ? 'Activo' : 'Inactivo'
            }));
